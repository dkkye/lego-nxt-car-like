...
class Converter:
  ...
  def sub_cb(self, msg):
	if (msg.data[:4]=="odom"):
	  rospy.loginfo('Callback odometry '+ msg.data)
	  tmp = msg.data[5:]
	  i_tz = tmp.find(";") 
	  encoderBackWheel = float(tmp[:i_tz])
	  tmp = tmp[i_tz+1:]
	  i_tz = tmp.find(";")
	  strEncoderSteering = tmp[:i_tz]
	  rospy.loginfo('Steering angle ' + strEncoderSteering)
	  encoderSteering = float(strEncoderSteering)
	  deltaEncoderBackWheel = encoderBackWheel - self.lastEncoderBackWheel
	  self.lastEncoderBackWheel = encoderBackWheel;
	  deltaTravel = 2*math.pi*self.radiusWheel*deltaEncoderBackWheel*self.ratioGearBackWheel/360
	  steeringAngle = 2*math.pi*encoderSteering*self.ratioGearSteering/360
	  if (steeringAngle == 0):
	    deltaAngle = 0
	    radiusCurve = 'nan'
	  else:
	    radiusCurve = self.wheelBaseWidht/2 + self.wheelBaseLenght/math.sin(steeringAngle)
