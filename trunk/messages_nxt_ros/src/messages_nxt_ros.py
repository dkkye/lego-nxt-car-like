...
class MessagesReader(Device):
	...
	self.comm = comm
        ...
        # create publisher
        self.pub = rospy.Publisher(params['name'], String)
        
    def trigger(self):
        ct = String()
	ct.data = str(self.comm.message_read(1,1, True)[1])
	if ct.data<>"":
	    self.pub.publish(ct)

class MessagesWriter(Device):
    def __init__(self, params, comm):
        Device.__init__(self, params)
        # create messages reader
	self.comm = comm
        # create subscriber
        self.sub = rospy.Subscriber(params['name'], String, self.msg_cb, None, 2)
    
    def msg_cb(self, msg):
        if msg.data <>0:
	    self.comm.message_write(5, msg.data)

    def trigger(self):
	self.t = 1

def main():
    rospy.init_node('messages_nxt_ros')
    ns = 'nxt_robot'
    host = rospy.get_param("~host", None)
    sock = nxt.locator.find_one_brick(host)
    b = sock.connect()
    b.start_program("NXT_ROS.rxe");	
    ...
    for c in config:
        ...
        elif c['type'] == 'MessagesReader':
            components.append(MessagesReader(c, b))
        elif c['type'] == 'MessagesWriter':
            components.append(MessagesWriter(c, b))
        else:
