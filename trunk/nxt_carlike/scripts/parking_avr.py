...
class Converter:
	...
	EnableControlDirection = 0
	if msg.range < self.depth_park_place/2+self.safety_dist:
	  self.secondBigRange = 0 
	  if self.countRanges< 2:
	    self.sumRanges = self.sumRanges + msg.range
	    self.countRanges = self.countRanges+1
	  else:
	    CurAvrRange = self.sumRanges /self.countRanges
	    self.sumRanges = 0
	    self.countRanges = 0	    
	    #if self.lastAvrRange <> 0:
	    EnableControlDirection = 1
	    differenceRange = CurAvrRange - 1.1*self.safety_dist
	    self.lastAvrRange = CurAvrRange
	else:
	  if self.secondBigRange == 1:
	    self.sumRanges = 0
	    self.countRanges = 0
	  else:
	    self.secondBigRange = 1
	if EnableControlDirection == 1 and self.state==1:
            CD = CarDriveStamped();
            CD.control.steering_angle = 6 * differenceRange;
            CD.control.speed = 2;
            rospy.loginfo('Control direction Angle='+str(CD.control.steering_angle)+' DiffRange='+str(differenceRange) + ' AvrRange='+str(CurAvrRange) )
            self.pub.publish(CD);	  
	...
	#rospy.loginfo('xc-xt='+ str(abs(self.xc-self.xt))+ 'yc-yt=' + str(abs(self.yc-self.yt)));
        if abs(self.xc-self.xt)<0.04 and abs(self.yc-self.yt)<0.04 and self.state==2 and self.n_manevr ==1:
	...
	if abs(self.xc-self.xt)<0.04 and self.state==2 and self.n_manevr ==2:
	    ...
	    self.state =0
	    self.pub_park.publish("target")
	if self.thetac>0.06 and self.thetac<0.1  and self.state==2 and self.n_manevr ==3:
            rospy.loginfo('It is target x='+ str(self.xc)+ 'y=' + str(self.yc)+ 'theta='+ str(self.thetac));
	    self.state =0
	    self.pub_park.publish("target")
